...
SELECT a.name, b.name, b.headquarters FROM models AS a  
JOIN brands AS b
ON a.brand_id = b.brand_id
WHERE a.name = 'Mustang';
...
SELECT count(model_id) FROM models WHERE brand_id = 'for';
...
select m.year, m.name, m.model_id, a.name FROM models AS m LEFT JOIN awards AS a ON m.model_id = a.winner_id WHERE m.year > 1963;
...
SELECT b.name, b.founded FROM brands AS b LEFT JOIN models AS m USING ( brand_id) GROUP BY brand_id HAVING count(model_id) = 0;
